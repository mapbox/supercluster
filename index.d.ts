...
// An individual point's tags are its input feature's `properties`, passed through verbatim, so they
// carry the spec's `null` as well as the `undefined` of a feature that omitted the member entirely.
// Same passthrough as geojson-vt's `Tags`, and callers writing tags out have to handle both.
export type TileTags<C, P> = (ClusterProperties & C) | P | null | undefined;
...
export default class Supercluster<
    P extends GeoJsonProperties = AnyProps,
    C extends GeoJsonProperties = AnyProps,
> {
    constructor(options?: Options<P, C>);

    options: Readonly<Required<Options<P, C>>>;

    /** Loads an array of point (or multi-point) features. Once loaded, the index is immutable. */
    load(points: InputFeature<P>[]): this;

    /** Clusters and points for the given bbox (`[westLng, southLat, eastLng, northLat]`) and zoom. */
    getClusters(bbox: BBox, zoom: number): (ClusterFeature<C> | PointFeature<P>)[];
    ...
    getTileRaw(zoom: number, x: number, y: number): RawTile<C, P> | null;

    /** The children of a cluster on the next zoom level. Throws if `clusterId` doesn't exist. */
    getChildren(clusterId: number): (ClusterFeature<C> | PointFeature<P>)[];
